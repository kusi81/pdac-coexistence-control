...
**Why this fits *PLOS Computational Biology*.** The paper is method-driven, fully reproducible, and explicitly hypothesis-generating rather than clinical: every output is framed as a candidate target, combination, or schedule to focus—not replace—experimental validation. It couples real single-cell spatial transcriptomics (Xenium and CosMx) to a mechanistic multiscale model, and it foregrounds methodological rigor and honest limitation-mapping over positive-result framing. Specifically, the model is grounded on real patient tissue as initial conditions, compares agents and schedules in a fair factorial design, resolves food-level entities toward standardized active ingredients (e.g., garlic → S-allylcysteine), and reports both one-at-a-time and global (Sobol) sensitivity analyses, which identify tumor proliferation and cytotoxic T-cell killing as the dominant controls of tumor burden while the stromal-barrier parameters govern which stromal strategy is optimal. Where results did not support the initial hypothesis, we say so and re-frame accordingly.

**Openness.** All analyzed datasets are from public repositories (GEO GSE274673; Mendeley Data doi:10.17632/kx6b69n3cb.1), and all analysis and modeling code is openly available at https://github.com/kusi81/pdac-coexistence-control and permanently archived on Zenodo (doi:10.5281/zenodo.21521806), so that every figure is reproducible from the exact commit. A preprint of this work is being posted to bioRxiv concurrently with submission.

**Declarations.** This manuscript is original, has not been published elsewhere, and is not under consideration by another journal. The author received no specific funding for this work and declares no competing interests. A large language model (Anthropic Claude, via the Claude Code interface) was used as a coding and writing tool—for implementing analysis and simulation code, running literature searches, and drafting text—and this is disclosed in full in the manuscript under "Use of AI-assisted technology". The AI system is not an author; I made all scientific and interpretive decisions, verified the code, analyses and references, and take full responsibility for the work. As an independent, unaffiliated researcher, I am actively seeking a collaborating laboratory to pursue the experimental validation ladder the paper lays out; this does not affect the integrity or availability of the computational work presented here.
